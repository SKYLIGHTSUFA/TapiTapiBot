...
- Ищет Tapitapi в номенклатуре без учета регистра: `tapitapi`, `тапитапи`.
...
Бот отправляет QR-строку чека (`qrraw`) в API только после OCR-проверки, что в чеке есть `tapitapi` или `тапитапи`. Это экономит суточный лимит API.

Билеты создаются только по позициям API, где в названии товара есть `tapitapi` или `тапитапи` без учета регистра.

Порядок распознавания:

1. DeepSeek-OCR-2 распознает чек и ищет строки номенклатуры с `tapitapi` или `тапитапи`.
